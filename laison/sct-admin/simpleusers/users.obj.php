...
$this->mysqli->set_charset("utf8");
			...
			$this->stmt->bind_result($id, $name, $color, $type);
		...
		public function addColor( $cu, $cc, $type )
		{

			$this->mysqli->set_charset("utf8");
			$sql = "INSERT INTO color (cu, cc, type) VALUES (?, ?, ?)";
			if( !$this->stmt = $this->mysqli->prepare($sql) )
				throw new Exception("MySQL Prepare statement failed: ".$this->mysqli->error);

			$this->stmt->bind_param("sss", $cu, $cc, $type);
			if( $this->stmt->execute() )
				return $this->stmt->insert_id;
		...
		{
$this->mysqli->set_charset("utf8");
			$sql = "DELETE FROM color WHERE id=? LIMIT 1";
			if( !$this->stmt = $this->mysqli->prepare($sql) )
				throw new Exception("MySQL Prepare statement failed: ".$this->mysqli->error);

			$this->stmt->bind_param($id);
			$this->stmt->execute();
	...
	public function addColorMotif($name, $color )
		{

$this->mysqli->set_charset("utf8");
			$sql = "INSERT INTO motif (name,color) VALUES (?, ?)";
			if( !$this->stmt = $this->mysqli->prepare($sql) )
				throw new Exception("MySQL Prepare statement failed: ".$this->mysqli->error);

			$this->stmt->bind_param('ss', $name, $color);
			if( $this->stmt->execute() )
				return $this->stmt->insert_id;
