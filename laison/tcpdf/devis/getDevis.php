...
$email=$_POST['email'];
$tel=$_POST['tel'];
$adress=$_POST['adress'].', '.$_POST['cp'].' '.$_POST['ville'];
$metres=$_POST['metres'];
$refcum=$_POST['refcum'];
$refcub=$_POST['refcub'];
...
<table width="100%" border="1px"><tbody>
...
<tr><td><h4>Type de projet</h4> </td><td>$projectType </td></tr>
...
/*$message = Swift_Message::newInstance('Demande de devis')
  ...
  ;*/
if($email == 'test10@keole.net'){
    $message = Swift_Message::newInstance('Demande de devis')
        ->setFrom(array($email => $namprenom))
        ->setTo(array('j0@keole.net'))
        ->setBody('Ci-joint vous trouverez une nouvelle demande de devis<br/>PJ: '.$pg, 'text/html')
        ->attach(Swift_Attachment::fromPath('../../devis/'.$name.'.pdf'))
    ...
    $message = Swift_Message::newInstance('Demande de devis')
        ->setFrom(array($email => $namprenom))
        ->setTo(array('info@cimenterie-de-la-tour.com'))
        ->setBody($html.'Ci-joint vous trouverez une nouvelle demande de devis<br/>PJ: '.$pg, 'text/html')
        ->attach(Swift_Attachment::fromPath('../../devis/'.$name.'.pdf'))
